...
// NEW: Lab1
// choose which half of the display to use
// and output a "message" to it
// Inputs: device 0/1 = top/bottom, line 0-3
//   string = null term. string, value = number
//   to display.
// Outputs: none
int hLine = 0;
void ST7735_Message (int device, int line, char *string, int32_t value)
{
	int offset = (8*device) + line; // device can only be 0 or 1
	
	if (!hLine)
	{
		
	  ST7735_DrawFastHLine(0, 75, ST7735_TFTWIDTH, ST7735_WHITE);
	}
	ST7735_SetCursor(0, 0+offset); //Beginning of the top screen
	
	
	ST7735_OutString(string); //output the value
	if (value>=0) // this may cause problems, fine for now
	{ 
